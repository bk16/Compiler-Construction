...
PTNode * addToTree( symbol s, tokenInfo * t, listNode * list, PTNode * pTree)
{
	...
	{
		pTree = addToLMNonTerminal( s, t, list, pTree );
		printf("In 2\n");
	}
...
PTNode * addToLMNonTerminal( symbol s, tokenInfo * t, listNode * list, PTNode * pTree)
{
	...
	if( isANonTerminal( s ))
	{
		...
		else
		{
			...
			{
				pTree = addToLMNonTerminal( s, t, list, pTree->children[i] );		
				i++;
			}
	...
	else
	{
		if( pTree->children == NULL )
		{
			...
			else
			{
				PTNode * temp;
				temp = pTree->parent;
				pTree = addToLMNonTerminal( s, t, list, temp);	
			printf("In 6\n");
