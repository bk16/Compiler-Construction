...
grammarNode ** populateGrammar(grammarNode ** G)
{
	...
	/*check this rule!!*/
	/*fielddefinitions > fielddefinition fielddefinition morefields //changed.*/
	...
	/*allvar > var .
	  allvar > TK_RECORDID TK_DOT TK_FIELDID .//changed.*/
	...
	G[temp]->list[1]->s = TK_DOT;
 	...
 	G[temp]->list[1]->next->next = NULL; //not needed*/
...
listNode ** populateFirst( listNode ** First )
{
	...
	First[stmt] = addSymbolToList( First[stmt], TK_RECORDID );
	...
	First[assignmentstmt] = addSymbolToList( First[assignmentstmt], TK_RECORDID );
	...
	First[highprecedenceoperators] = addSymbolToList( First[highprecedenceoperators], TK_MUL );
	First[highprecedenceoperators] = addSymbolToList( First[highprecedenceoperators], TK_DIV );
	First[lowprecedenceoperators] = addSymbolToList( First[lowprecedenceoperators], TK_PLUS );
	First[lowprecedenceoperators] = addSymbolToList( First[lowprecedenceoperators], TK_MINUS );
...
listNode ** populateFollow( listNode ** Follow )
{
	...
	Follow[typedefinitions] = addSymbolToList( Follow[typedefinitions], TK_RECORDID );
	...
	Follow[typedefinition] = addSymbolToList( Follow[typedefinition], TK_RECORDID );
	...
	Follow[newstate] = addSymbolToList( Follow[newstate], TK_ASSIGNOP );	
	...
	Follow[highprecedenceoperators] = addSymbolToList( Follow[highprecedenceoperators], TK_MUL );
	Follow[highprecedenceoperators] = addSymbolToList( Follow[highprecedenceoperators], TK_DIV );
	Follow[highprecedenceoperators] = addSymbolToList( Follow[highprecedenceoperators], TK_PLUS );
	Follow[highprecedenceoperators] = addSymbolToList( Follow[highprecedenceoperators], TK_MINUS );
	Follow[highprecedenceoperators] = addSymbolToList( Follow[highprecedenceoperators], TK_SEM );
	Follow[lowprecedenceoperators] = addSymbolToList( Follow[lowprecedenceoperators], TK_PLUS );
	Follow[lowprecedenceoperators] = addSymbolToList( Follow[lowprecedenceoperators], TK_MINUS );
	Follow[lowprecedenceoperators] = addSymbolToList( Follow[lowprecedenceoperators], TK_SEM );
	...
	Follow[all2] = addSymbolToList( Follow[all2], TK_MUL );
	Follow[all2] = addSymbolToList( Follow[all2], TK_DIV );
	Follow[all2] = addSymbolToList( Follow[all2], TK_PLUS );
	Follow[all2] = addSymbolToList( Follow[all2], TK_MINUS );
	Follow[all2] = addSymbolToList( Follow[all2], TK_SEM );
...
listNode *  getFirstOfAlpha( listNode * node, listNode ** First , listNode ** Follow  )
{
	...
	if( isATerminal( node->s ) )
	{
		newList = getListNodeMemory();
		newList = addSymbolToList( newList, node->s );
		return newList;
	}
	...
	while( node != NULL )
	{
		newList = addSymbolsToList( newList, First[node->s] );
		if( containsEpsilon( First[node->s] ) )
		{
			node = node->next;
		}
		else
...
int getSizeList( listNode * list )
{
	...
	return 1 + getSizeList(list->next);
}
	...
	*TT = T;
	...
	for(  i=0; i<NUMBER_OF_NONTERMINALS ; i++)
	{
		...
		for( j=0; j<numberOfRules ; j++)
		{
			...
			if( containsEpsilon( first ) && checkIfListContainsSymbol( Follow[i] , TK_DOLLAR) )
			{
...
parseTree  parseInputSourceCode(char *testcaseFile, table T)
{
	...
	tokenInfo tokenArray[ numberOfTokens ];
	...
	while( !empty( stack ) )
	{
		...
		///printf("Stack top is : %s\n", tokenNameFromSymbol(stackTop));
		printf("Stack is : ");
		print( stack );

		if( isANonTerminal( stackTop ) )
		{
			listNode * list = T[stackTop][ tokenArray[next].s ];
			...
			else
			{
				...
				PT = addChildrenToNode(list, stackTop,PT );
			}*/
			
		}
		else	/*stack top is a terminal*/
		{
			if( stackTop == tokenArray[next].s )
			{
